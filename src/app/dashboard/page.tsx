...
import { PersonalSettings } from "@/components/dashboard/personal-settings";
...
import { getPersonalProfile } from "@/lib/personalization";
...
export default async function DashboardPage() {
  const [user, repository, store, profile] = await Promise.all([getCurrentUser(), getWorkspaceRepository(), getStore(), getPersonalProfile()]);
  ...
  return <main className="dashboard-shell">
    <header className="dashboard-header"><div><p>{language.dashboard.workspace}</p><h1>{profile.displayName}</h1></div>
    ...
    <section className="dashboard-grid" aria-label={language.dashboard.overview}>
      ...
      <article className="dashboard-panel dashboard-wide"><p className="dashboard-kicker">{language.dashboard.profileSettings}</p><h2>{user?.email ?? profile.displayName}</h2>
        <PersonalSettings initialProfile={profile} providers={providerOptions()} /></article>

      <article className="dashboard-panel"><p className="dashboard-kicker">{language.dashboard.memory}</p><h2>{language.dashboard.activeKnowledge}</h2><p className="dashboard-value">{memories.length}</p><p className="dashboard-empty">{language.dashboard.activeKnowledgeCopy}</p></article>
      <article className="dashboard-panel"><p className="dashboard-kicker">{language.dashboard.usage}</p><h2>{language.dashboard.providerCalls}</h2><p className="dashboard-value">{usage.length}</p><p className="dashboard-empty">{language.dashboard.providerCallsCopy}</p></article>
      <article className="dashboard-panel"><p className="dashboard-kicker">{language.dashboard.intelligence}</p><h2>{language.dashboard.provider}</h2><p className="dashboard-value">{settings.provider}</p><p className="dashboard-empty">{language.dashboard.providerControlCopy}</p></article>
      ...
      <article className="dashboard-panel"><p className="dashboard-kicker">{language.dashboard.briefArchive}</p><h2>{language.dashboard.dailyBriefs}</h2><p className="dashboard-value">{briefs.length}</p><p className="dashboard-empty">{language.dailySummary.archive}</p></article>
      <article className="dashboard-panel"><p className="dashboard-kicker">{language.dashboard.trajectoryTimeline}</p><h2>{language.dashboard.observations}</h2><p className="dashboard-value">{history.length}</p><p className="dashboard-empty">{language.dashboard.observationsCopy}</p></article>
    </section>
  </main>;
}
