...
interface BriefResponse {
  requestId: string;
  speech: string;
  signal: ExecutiveSignalResponse;
  diagnostics?: { provider: string; persisted: boolean };
}
...
const diagnostic = (event: string, detail: Record<string, unknown> = {}) =>
  console.info("[trajectory:voice]", { event, at: new Date().toISOString(), ...detail });
...
export function TrajectoryExperience({
  ...
  const speakOnce = useCallback((requestId: string, text: string) => {
    if (spokenRequestIdRef.current === requestId || !("speechSynthesis" in window)) {
      setStatus("idle");
      return;
    }
    ...
    utterance.onstart = () => {
      diagnostic("speech_started", { requestId, playbackCount: 1 });
      setStatus("speaking");
    };
  ...
  const submitTranscript = useCallback(async () => {
    ...
    if (!spokenPrompt) {
      diagnostic("transcript_rejected", { reason: "empty" });
      setStatus("idle");
      return;
    }
    if (requestInFlightRef.current || submittedTranscriptRef.current === spokenPrompt) {
      diagnostic("duplicate_request_guard", { inFlight: requestInFlightRef.current });
      return;
    }
  ...
  const listen = useCallback(() => {
    ...
    recognition.onend = () => {
      ...
      if (accepted) {
        diagnostic("transcript_accepted", { transcriptLength: accepted.length });
        void submitTranscript();
      } else {
        diagnostic("transcript_rejected", { reason: "empty_on_end" });
        setStatus("idle");
      }
    };
    recognition.onerror = (event) => {
      ...
      if ((finalTranscriptRef.current || transcriptRef.current).trim()) void submitTranscript();
        ...
        setRecoverableError(language.voice.failure);
        setStatus("failure");
      }
    };
  ...
  const statusLabel = status === "idle"
    ? language.status.observingQuietly
    : status === "listening"
      ? language.status.listening
      : status === "speaking"
        ? language.status.speaking
        : status === "failure"
          ? language.voice.failure
          : status === "unsupported"
  ...
  const interactionHint = status === "unsupported"
    ? language.voice.unavailableInBrowser
    : status === "listening"
      ? language.voice.tapToStop
      : active
        ? null
        : language.voice.tapToSpeak;

  return (
    <main className={`trajectory-experience light-${lightTone[displayedRisk]} status-${status}`}>
      <div className="star-field" aria-hidden="true">
        <div className="stars stars-near" /><div className="stars stars-far" /><div className="milky-way" /><div className="cosmic-dust" /><div className="nebula" />
        <div className="distant-galaxy galaxy-one" /><div className="distant-galaxy galaxy-two" /><div className="shooting-star shooting-star-one" /><div className="shooting-star shooting-star-two" />
      </div>
        ...
        <div className="header-controls">
          <label className="provider-setting">
            ...
            <select value={provider} onChange={(event) => chooseProvider(event.target.value as ProviderPreference)} aria-label={language.brand.providerLabel}>
              ...
              {providers.map((option) => <option key={option.id} value={option.id} disabled={!option.configured}>{option.label}{option.configured ? "" : language.brand.unavailableSuffix}</option>)}
            </select>
          </label>
      ...
      <section className="intelligence-stage" id="intelligence" aria-label={language.brand.intelligenceRegion}>
        <button ref={orbRef} type="button" className={`orb-system is-${status}`} onClick={onOrbClick} disabled={status === "unsupported" || (active && status !== "listening" && status !== "speaking")} aria-label={status === "listening" ? language.voice.stopInteraction : language.voice.speakToTrajectory}>
          <div className="watch-stream stream-one" /><div className="watch-stream stream-two" /><div className="orb-halo halo-one" /><div className="orb-halo halo-two" /><div className="speech-wave speech-wave-one" /><div className="speech-wave speech-wave-two" />
        ...
        <div className="orb-copy" aria-live="polite">
          ...
          {["idle", "unsupported", "failure"].includes(status)
      ...
      <section className={`briefing-card${signal ? " signal-updated" : ""}`} aria-labelledby="briefing-title" aria-live="polite">
        ...
        <div className="briefing-details">
          ...
          <div><span>{language.headings.trajectoryLogic}</span><p>{displayedReasoning}</p></div>
          {signal ? <>
            <div><span>Confidence</span><p>{Math.round(signal.confidence * 100)}%</p></div>
            <div><span>Urgency</span><p>{Math.round(signal.urgency * 100)}%</p></div>
            <div><span>Suggested next action</span><p>{signal.suggestedNextAction}</p></div>
          </> : null}
        </div>
      </section>
    </main>
