...
export function VoiceMode() {
  ...
  const brief = useCallback(async () => {
    ...
    try {
      ...
      setStatus("idle");
      setLines([language.voice.failure]);
    }
  ...
  const statusCopy: Record<Status, string> = {
    idle: language.status.idle,
    loading: language.status.loading,
    speaking: language.status.speaking,
    listening: language.status.listening,
    unsupported: language.status.unsupported,
  };
  ...
  return (
    <div className="rounded-xl border border-border bg-surface/70 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted">
            {language.headings.voice}
          </h2>
          <p className="mt-0.5 text-xs text-faint">{language.voice.proactiveBrief}</p>
        </div>
        <Badge
          tone={status === "speaking" ? "accent" : status === "listening" ? "positive" : status === "unsupported" ? "negative" : "neutral"}
        >
          {statusCopy[status]}
        </Badge>
      </div>
      ...
      <div className="mt-3 flex gap-2">
        ...
        >
          {status === "loading" ? language.loadingStates.integratingObservations : language.voice.briefMe}
        </button>
        <button
          type="button"
          onClick={stop}
      ...
      {heard ? (
        <p className="mt-3 rounded-md border border-positive/30 bg-positive/5 px-3 py-2 text-xs text-muted">
          <span className="text-positive">{language.voice.heardPrefix}</span> {heard}
        </p>
      ) : null}
      ...
      {lines.length > 0 ? (
        <ul className="mt-3 space-y-1.5 border-t border-border/60 pt-3">
          {lines.map((line, index) => (
            <li key={index} className="text-xs leading-relaxed text-muted">{line}</li>
          ))}
        </ul>
      ) : (
        <p className="mt-3 text-xs leading-relaxed text-faint">{language.voice.empty}</p>
      )}

      {status === "unsupported" ? (
        <p className="mt-2 text-[11px] text-negative">{language.voice.unsupported}</p>
      ) : null}
    </div>
  );
}
